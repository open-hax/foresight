...
before use. The archive contains a regular-file checksum manifest, an explicit
symbolic-link target manifest, and a verifier for both. Node-based entry points
are bundle-relative wrappers, so direct invocation does not depend on a host
Node executable or on first changing `PATH`.
